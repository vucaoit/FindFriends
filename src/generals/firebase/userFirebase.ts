...
export const scanner = async (user:UserModel,onUpdate?:(user:UserModel)=>void) : Promise<PersonModel[] | null> => {
    try {
        getLocation((location=>{
          if(user.user){
          user = {...user,location : (location ?location:undefined)}
...
export const updateUserInfo = async (user:UserModel)=>{
    try{
      await firestore()
                .collection('users')
                ...
                .set({
                    ...
                    location:user.location?user.location:{latitude: 0, longitude: 0},
